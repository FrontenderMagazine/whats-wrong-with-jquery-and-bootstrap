...
*Цель этой статьи — доступно рассказать о том, когда и почему следует использовать 
jQuery и Bootstrap. Я написал её не для разжигания холиваров, мне хотелось максимально 
прояснить вопрос необходимости этих инструментов. Но если у вас всё же возникнет 
желание с негодованием опровергнуть её содержимое и хорошенько поспорить — сначала 
прочтите статью до конца, а затем выскажитесь в комментариях.*
...
Это JavaScript-библиотека, вышедшая в свет в далеком 2006 году (другие
...
космос! Она позволяет работать с DOM, навешивать на элементы события,
...
из-за того, что им это интересно. Они изучают всё сами. В вузах не учат делать
фронтенд, а качественных курсов очень мало. Зачастую новичок не знает, в какую
...
её этаким стандартом для разработки сайтов. Из-за этого у новичков
может сложиться впечатление, что JavaScript учить не нужно, достаточно уметь
...
механизмов JavaScript вы не сможете эффективно решать возникающие проблемы.

Если вы не знакомы с ним — начните его изучение с [прекрасного учебника на русском языке][0].
Даже английский учить не придётся, вот радость-то! Не беритесь за jQuery, 
пока не освоите основы языка. Если у вас совсем нет опыта в программировании, 
начните с (тоже русскоязычных) курсов на [Хекслете][1]. Так вы заложите хорошую 
базу, с которой сможете без проблем изучить любую библиотеку или фреймворк. 
Без знания языка нет смысла пытаться освоить jQuery или что-нибудь другое.
...
браузеры — jQuery не нужен, потому что большинство возможностей этой библиотеки 
уже доступны нативно (или с полифиллами). Даже тратить время на изучение jQuery 
без осознанной необходимости не нужно: лучше потратьте его на изучение нативных API.
...
вёрстки. CSS-фреймворк предоставляет разработчику инструменты для написания кода,
а не сам код. Bootstrap же, по большей части, как раз предоставляет готовый код. 
Неудивительно, ведь изначально он был набором визуальных компонентов твиттера. 
Да, в Bootstrap есть всякие вспомогательные классы и примеси для препроцессоров, 
но его основа — именно *готовая вёрстка*.
...
количеством костылей в стилях. Bootstrap ведь не следует никакой методологии,
вроде БЭМ, — его стили очень сильно завязаны на контексте и каскаде. Рано или
...
согласно дизайну. Кстати, новички не всегда знают о том, что из Bootstrap
...
иконочных шрифтов и jQuery-плагинов. Неиспользуемый код — это плохо: он является
потенциальным источником проблем и багов в будущем, это лишние килобайты, которые
